...
def _do_exclusion(probs, list_threshold):
...
def postprocess_exclusion(list_probs, list_threshold):
    ...
    if list_probs.ndim == 1:  # one image's probabilities
        probs = _do_exclusion(list_probs, list_threshold)
        return probs

    if list_probs.ndim == 2:  # probabilities of a list of images
        for i in range(len(list_probs)):
            list_probs[i] = _do_exclusion(list_probs[i], list_threshold)
        return list_probs
...
THRETHOLD_ALLNEGATIVE_CLASS0 = my_config.THRETHOLD_ALLNEGATIVE_CLASS0

def _do_all_negative(probs, list_threshold):
    num_positive = 0
...
def postprocess_all_negative(list_probs, list_threshold):
    ...
    if list_probs.ndim == 1:  # one image's probabilities
       return _do_all_negative(list_probs, list_threshold)

    if list_probs.ndim == 2: # probabilities of a list of images
        for i in range(len(list_probs)):
            list_probs[i] = _do_all_negative(list_probs[i], list_threshold)

        return list_probs

#endregion

#region both non-referable label and other referable labels are positive
